<?php
  ...
  if (!preg_match("/^[0-9]{2,4}$/",$company,$ms)){print "ERROR: Wrong company code provied: $company\n"; return;}
  if (!preg_match("/^([2-9][0-9]|[1-9][0-9][0-9])$/",$amount,$ms)){print "ERROR: Wrong amount provied: $amount\n"; return;}
  if (!preg_match("/^[1-9][0-9]{0,2}$/",$mtype,$ms)){print "ERROR: Wrong mtype provied: $mtype\n"; return;}
  $data{'number'}=$number;
  ...
  $xml=$we->request("2", $data, $detail=0, $test=0);
